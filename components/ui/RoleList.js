...
const RoleList = ({ value, onChange }) => {
  ...
  const { data, error, mutate } = useSWR(
    `/v1/branches/${selectedBranch.id}/roles`,
    fetcher
  )
  const filteredRoles =
    query === ""
      ? data?.roles
      : data?.roles?.filter((option) => {
          return option.toLowerCase().includes(query.toLowerCase())
        })

  const handleDeleteDeptmnt = async (role) => {
    try {
      await APIService.delete(
        `/v1/branches/${selectedBranch.id}/roles?name=${role}`
      )
      toast.success("Role Deleted")
      mutate()
      if (role === value) onChange("")
    } catch (error) {
  ...
  const handleAddDeptmnt = async (role) => {
    if (!role) return
    setLoading(true)
    try {
      await APIService.post(`/v1/branches/${selectedBranch.id}/roles`, {
        name: role,
      })
  ...
  if (error) {
    if (error.code === "ERR_NETWORK") {
      ...
      return <span>{"Can't fetch Roles"}</span>
    }
  }
        ...
        {({ open }) => (
          <>
            <div className="flex justify-between items-center bg-background rounded-2xl h-[50px] outline-2 outline-accent">
              <Combobox.Input
                placeholder={value || `Select Role (${data.roles.length})`}
