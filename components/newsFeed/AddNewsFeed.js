...
const AddNewsFeed = ({ onClose }) => {
  ...
  const { file, onDiscard, onSelect } = useFileReader("image/*")
  ...
  const handleSubmit = async () => {
    setIsLoading(true)
    try {
      await APIService.post(`/v1/branches/${selectedBranch.id}/announcements`, {
        title,
        text,
        department: dep,
      })
      toast.success("Announcements Added")
    } catch (error) {
  ...
  return (
    <div className="px-16 py-12 bg-white rounded-2xl">
      <h3 className="text-4xl leading-6 font-medium mb-14">New Announcement</h3>
